...
13. **`users`** (**MODIFICACIÓN DE TABLA EXISTENTE**)
    * *Nota:* Esta tabla YA EXISTE. Debes crear una migración para **RENOMBRAR** sus columnas y agregar las nuevas. NO LA BORRES.
    ...
    * *Mantener compatibilidad renombrando:*
...
2.  **Sincronización Atómica (Dual Write):**
    ...
    * *Resultado:* El cambio de permiso es efectivo en el siguiente request sin reiniciar el servidor.
3.  **Exposición:** Asegura que estos métodos sean accesibles para ser llamados por futuros Business Objects.

### FASE 3: QA, Tests y Documentación
1.  **Tests:** Repara los tests de Auth que fallarán por el cambio de campos.
2.  **Docs:** Genera un Markdown explicando el nuevo esquema ER y cómo funciona el sistema de seguridad dinámico.

---

## 5. TU PRIMERA TAREA (ANÁLISIS)

**NO escribas código final todavía.**
Quiero que analices los archivos mencionados y me presentes un **PLAN DE ACCIÓN DETALLADO (Markdown)** para la **FASE 1**.

Tu plan debe incluir:
1.  El código SQL exacto (o código de migración TypeScript) que planeas ejecutar para transformar la tabla `users`.
2.  Una lista de los archivos TypeScript específicos que has detectado que se romperán y necesitan refactorización.
3.  Confirmación de que has entendido el esquema de base de datos completo.
